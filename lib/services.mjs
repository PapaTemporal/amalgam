 ...
 */
/**
 * Whether the installed runtime could use a GPU even if asked to.
 *
 * The opt-in and the capability are different questions, and only one of them
 * was being asked. The Windows build published on the release is llama.cpp's
 * CPU-only package: every backend beside it is a ggml-cpu-*.dll, and there is
 * no Vulkan, CUDA or HIP library in the directory. Turning the opt-in on
 * against it produces `-ngl 999` for a binary with nowhere to put the layers,
 * which does not fail — it quietly runs on the CPU while every surface reports
 * that offload is enabled.
 *
 * So the flags follow what the runtime can actually do, and `gpu status` says
 * which of the two is missing. macOS is not in this position: its build has
 * Metal compiled in, so the presence of the server is the presence of the
 * capability.
 */
export function gpuCapable() {
  if (process.platform === "darwin") return true;   // Metal ships in the build
  try {
    const dir = path.dirname(llamaServerPath());
    return fs.readdirSync(dir).some((f) => /ggml-(vulkan|cuda|hip|sycl)/i.test(f));
  } catch {
    return false;
  }
}
...
export function deviceArgs() {
  // Deliberately NOT gated on gpuCapable(): the opt-in is what turns this on,
  // and a constraint says so. On a CPU-only build the two flag sets behave
  // identically anyway, so making the flags conditional would buy nothing and
  // would mean a capable runtime dropped in later did not simply work. What
  // the capability check is for is saying so — see `amalgam gpu status`.
