 ...
 *
 * Kept beside the timings rather than in them, because one is a measurement
 * and the other is a decision. The environment variable still wins when it is
 * set: something explicitly exported for one run should not be overruled by a
 * preference saved months ago, and a machine that must never start work on
 * its own can be held that way from the outside.
 */
const SETTINGS = path.join(HOME, "refresh-settings.json");
...
export const autoRefreshEnabled = () => {
  const env = process.env.AMALGAM_AUTO_REFRESH;
  if (env) return env.toLowerCase() !== "off";
  const stored = settings().autoRefresh;
  return stored === undefined ? true : !!stored;
};

/** Whether the environment is holding the switch, so the interface can say so. */
export const autoRefreshForced = () => !!process.env.AMALGAM_AUTO_REFRESH;

export function setAutoRefresh(on) {
  try {
    fs.mkdirSync(HOME, { recursive: true });
    fs.writeFileSync(SETTINGS, JSON.stringify({ ...settings(), autoRefresh: !!on }, null, 2));
...
/**
 * What the last automatic refresh actually did.
 *
 * The refresh runs detached from a session-end hook, with its stdout going to
 * nowhere on purpose — nobody wants a rebuild printing into their terminal
 * after they have closed the session. The cost of that is silence, and silence
 * is how this feature spent its whole life broken: the hook spawned a path
 * that did not exist, the child died instantly, and every surface that could
 * have said so was reporting on what the policy INTENDED rather than on what
 * happened.
 *
 * So each run writes down that it ran. One line, overwritten each time —
 * enough to tell "never ran" from "ran and found nothing to do", which are
 * indistinguishable from the outside and mean completely different things.
 */
export function recordRun({ considered = 0, rebuilt = [], error = null } = {}) {
  const data = load();
  data.$lastRun = { at: new Date().toISOString().replace("T", " ").slice(0, 19), considered, rebuilt, error };
  save(data);
}

export const lastRun = () => load().$lastRun ?? null;
...
export function dueForRefresh() {
  ...
  // Deliberately the plan, filtered — rather than the same walk written twice.
  // When they were separate, a repository registered both in its own right and
  // inside a workspace was listed once and rebuilt twice.
  return refreshPlan().filter((r) => r.refresh);
}

/** The same question answered for everything, for showing rather than doing. */
export function refreshPlan() {
  const out = [];
  // A repository can be reached twice — registered in its own right and also
  // sitting inside a registered workspace — and it is still one repository
  // with one graph. Listing it twice would double every estimate and read as
  // though it were going to be built twice.
  const seen = new Set();
  for (const proj of readRegistry().projects) {
    ...
    for (const part of parts) {
      const at = path.resolve(part.path);
      if (seen.has(at)) continue;
      seen.add(at);
      const record = buildRecord(at);
      out.push({
        name: part.name,
        path: part.path,
