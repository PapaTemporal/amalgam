...
function projectSummary(dir) {
  ...
  // Why a graph is absent matters more than the fact of it: "never built" and
  // "cannot be built here" call for completely different actions, and a card
  // that says only "none" sends people to press a button that cannot work.
  //
  // The case that looks most like a bug: a folder that HOLDS repositories is
  // not itself a project. Building a graph there indexes each service under
  // its own path, so the parent never gains one however many times the button
  // is pressed — and check detection finds nothing either, because there is no
  // manifest at that level. Both symptoms at once, and neither explains
  // itself.
  const services = exists && !fs.existsSync(path.join(repo, ".git"))
    ? fs.readdirSync(repo, { withFileTypes: true })
        .filter((e) => e.isDirectory() && !e.name.startsWith(".")
          && fs.existsSync(path.join(repo, e.name, ".git")))
        .map((e) => ({ name: e.name, path: path.join(repo, e.name) }))
    : [];
  const isWorkspace = services.length >= 2;

  const graphBlocked = isWorkspace
    ? `This folder holds ${services.length} repositories rather than being one. A graph belongs to each service, not to the folder above them.`
    : !has("uv")
      ? "uv is not installed, and graphify needs it — see https://docs.astral.sh/uv/"
      : !exists ? "the folder is gone" : null;
  ...
  return {
    ...
    graph: index ? { symbols: index.symbols, edges: index.edges, importedAt: index.imported_at } : null,
