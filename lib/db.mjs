...
export function open() {
  ...
  db.exec(SCHEMA);
  // Vector columns are added by migration rather than in SCHEMA so existing
  // stores pick them up without a rebuild.
  for (const [table, col] of [["l1_facts", "embedding"], ["l2_scenarios", "embedding"]]) {
    const cols = db.prepare(`PRAGMA table_info(${table})`).all().map((c) => c.name);
    if (!cols.includes(col)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${col} BLOB`);
  }
  return db;
}
