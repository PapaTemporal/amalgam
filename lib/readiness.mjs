...
import { agentCli, copilotCli } from "./session.mjs";
 ...
 * than to this one.
 */
export function machineGaps({ claude = agentCli(), copilot = copilotCli() } = {}) {
  const gaps = [];

  if (!claude) {
    // Telling somebody who is looking at an installed Copilot that they have
    // no agent CLI is both wrong and unhelpful. What they are missing is the
    // one amalgam can drive, and the tools they already have are worth saying.
    gaps.push({
      id: "agent",
      what: copilot
        ? "Copilot is here, but sessions need Claude Code"
        : "No agent CLI on this machine",
      why: copilot
        ? "amalgam speaks Claude Code's streaming protocol to run a session. Copilot is a " +
          "different program with a different one. Everything else still works: " +
          "`amalgam wire --copilot` puts these tools inside Copilot, where you already are."
        : "Without one the interface can only compose prompts for you to paste. Nothing can be run here.",
      fix: "npm install -g @anthropic-ai/claude-code",
      action: { endpoint: "/agent/install", label: "Install it" },
      // Installing a global npm package on somebody's behalf, mid-update, is
      // not a thing to do quietly — and it would still leave the sign-in.
      auto: false,
      // Installing it is not the same as being able to use it, and there is no
      // way to know which without running it.
      note: copilot
        ? "Only needed to run work from this interface. Copilot users can stop here."
        : "Afterwards run `claude` once and sign in — that part cannot be done from here.",
    });
  }
  ...
  if (!visVendored()) {
    gaps.push({
      ...
      // The one gap worth closing without being asked: it is 686 KB, it is
      // already the kind of network call an update is making anyway, and
  ...
  for (const proj of readRegistry().projects) {
    ...
    for (const part of parts) {
      // Nothing to say about a repository with no graph — that is a choice,
      // not a gap. The old check looked for graph.json, which is now consumed
      // during indexing, so it would have reported every healthy repository as
      // having no graph at all.
      if (!isIndexed(part.path)) continue;
      if (!fs.existsSync(path.join(part.path, PAGE_REL))) {
  ...
  if (needs.length) {
    ...
    gaps.push({
      ...
      flag: "--build",
      auto: false,
      // Carried so `update --build` knows where to go without asking again.
      repos: [...new Set(needs.map((n) => n.project))].flatMap((root) => {
        const parts = isWorkspace(root) ? servicesOf(root) : [{ name: path.basename(root), path: root }];
        return parts.filter((part) => needs.some((n) => n.service === part.name && n.project === root));
      }),
