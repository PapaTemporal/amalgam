...
import { indexStatus } from "./graphdb.mjs";
...
/**
 * The merged graph, kept between requests.
 *
 * Building it means reading every symbol and every edge of every service out
 * of the index: a few milliseconds for a small project and several seconds for
 * one with a hundred thousand symbols. Exploring is many small questions in a
 * row — search, then a symbol, then its neighbourhood, then its impact — and
 * paying that price per question made the largest projects, the ones that most
 * need exploring, the ones least able to be.
 *
 * Held against what the index says it last imported, so a rebuild invalidates
 * it without anyone having to remember to.
 */
const graphCache = new Map();

function cachedGraph(root) {
  const stamp = graphStamp(root);
  const hit = graphCache.get(root);
  if (hit && hit.stamp === stamp) return hit.graph;
  const graph = graphFor(root);
  // Only the newest project is worth holding: these are large, and moving
  // between projects is rare compared with moving around inside one.
  graphCache.clear();
  if (graph) graphCache.set(root, { stamp, graph });
  return graph;
}

/** A cheap fingerprint of what is indexed, to know when the graph moved on. */
function graphStamp(root) {
  const parts = partsOf(root);
  return parts.map((p) => {
    const st = indexStatus(p.path);
    return st ? `${p.name}:${st.imported_at}:${st.symbols}` : `${p.name}:none`;
  }).join("|");
}

/** Drop it, for a caller that has just changed the index. */
export const forgetGraphCache = () => graphCache.clear();
  ...
  for (const part of parts) {
    ...
    for (const [key, members] of Object.entries(a.communities ?? {})) {
      ...
      communities.push({
        id: `${part.name}#${key}`,
        service: part.name,
        // The ids are not sent. A page shows a cluster's name, size and
        // cohesion; shipping every member turned this into a megabyte of JSON
        // for a large project, to draw four numbers.
 ...
 */
export function tree(root, { under = "", depth = 1 } = {}) {
  const g = cachedGraph(root);
  if (!g) return null;
  ...
  // Re-index against the collapsed tree.
  //
  // Collapsing a chain of single-child directories rewrites a node's name and
  // path — `api` holding only `src` becomes one row called `api/src`. The
  // index built while walking still points at the inner node, so asking for
  // the branch `api/src` returned something calling itself `src`: the right
  // children under the wrong name, which is the kind of small lie that makes a
  // tree feel broken without being obviously wrong.
  dirIndex.clear();
  (function reindex(node) {
    if (node.kind !== "symbol") dirIndex.set(node.path ?? "", node);
    (node.children ?? []).forEach(reindex);
  })(rootNode);

  // Only the branch asked for, only as deep as asked.
  //
  // The whole tree of a hundred thousand symbols is eleven megabytes of JSON,
  // which is not a tree anybody can browse — it is a download. A level at a
  // time is what a collapsed tree actually needs, and every row still carries
  // its total, so a folded branch says how much it is hiding.
  const start = under ? dirIndex.get(under) : rootNode;
  if (!start) return null;
  return prune(start, depth);
}

/** A node with its descendants trimmed to `depth` levels, counts intact. */
function prune(node, depth) {
  const shallow = {
    name: node.name, kind: node.kind, path: node.path ?? "",
    id: node.id, line: node.line, degree: node.degree,
    symbols: node.symbols,
    childCount: node.children?.length ?? 0,
  };
  if (node.kind === "symbol") return shallow;
  shallow.children = depth > 0 ? (node.children ?? []).map((c) => prune(c, depth - 1)) : [];
  // A branch that was trimmed rather than genuinely empty must say so, or a
  // collapsed tree looks like an empty one.
  shallow.hasMore = depth <= 0 && shallow.childCount > 0;
  return shallow;
}
