...
/**
 * The extractor, pinned.
 *
 * graphify ships several times a day — it was 0.9.51 with three releases on the
 * day this was pinned — so an unpinned `--from graphifyy` means two machines
 * building "the same" graph can get different symbol counts, and a rebuild can
 * change results for a reason nothing recorded. For a project that reports
 * measured numbers, an extractor that drifts underneath the measurement is the
 * loose thread.
 *
 * Bumping this is a deliberate act: change the version, rebuild, and expect the
 * numbers to move.
 */
export const GRAPHIFY_VERSION = "0.9.51";
export const graphifySpec = (extras = "") => `graphifyy${extras}==${GRAPHIFY_VERSION}`;

/**
 * Subcommands that read an existing graph rather than building one. Everything
 * else extracts, and extraction is what must never take the doc/image pass.
 */
const READ_ONLY_MODES = new Set(["explain", "path", "query", "cluster-only"]);

/**
 * Force local-only extraction, and refuse to run without it.
 *
 * `--code-only` restricts graphify to local tree-sitter AST parsing; the
 * doc/image semantic pass calls a cloud backend, which this stack forbids
 * outright. That rule was previously held up by a comment and one literal at
 * one call site — the weakest possible enforcement of the strongest constraint,
 * in a dependency that ships daily. Routing every invocation through here makes
 * the flag structural: a new call site gets it without knowing to ask, and an
 * extraction that somehow lacks it fails loudly instead of quietly reaching a
 * host nobody chose.
 */
export function graphifyArgs(cliArgs) {
  const args = [...cliArgs];
  if (READ_ONLY_MODES.has(args[0])) return args;
  if (!args.includes("--code-only")) args.push("--code-only");
  return args;
}
...
export function verifyEdge(repo, site, name, targetFile = "", cache = new Map()) {
  ...
  // An import may rename what it brings in — `import { open as openDb }` means
  // the call site says openDb(), and looking for open() would wrongly conclude
  // the call is not there at all.
  const local = localNames(lines, name, targetFile);
  const esc = local.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|");
...
/**
 * What this file actually calls the symbol. Usually its own name; where the
 * import renames it (`import { open as openDb }`), the alias as well.
 */
function localNames(lines, name, targetFile) {
  const names = [name];
  if (!targetFile) return names;
  const stem = targetFile.split(/[\\/]/).pop().replace(/\.[^.]+$/, "");
  const esc = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const aliased = new RegExp(`\\b${esc}\\s+as\\s+(\\w+)|\\b${esc}\\s*:\\s*(\\w+)`);
  for (const l of lines) {
    if (!/(?:import\s*\{|=\s*require\()/.test(l) || !l.includes(stem)) continue;
    const m = aliased.exec(l);
...
function boundLocally(lines, at, esc, targetFile) {
  const stem = targetFile.split(/[\\/]/).pop().replace(/\.[^.]+$/, "");
  // `esc` may be an alternation of the name and its local alias, so every use
  // of it has to be grouped — without the group, `const\s+open|openDb` parses
  // as "const open" OR "openDb", which matches almost anything.
  const any = `(?:${esc})`;
  const declaration = new RegExp(`\\b(?:const|let|var|function|class)\\s+${any}\\b`);
  const param = new RegExp(`\\(\\s*(?:\\w+\\s*,\\s*)*${any}\\s*(?:,\\s*\\w+\\s*)*\\)\\s*=>`);
  const fnParam = new RegExp(`function\\s*\\w*\\s*\\([^)]*\\b${any}\\b`);
  const namedImport = /(?:import\s*\{|=\s*require\()/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (namedImport.test(l) && new RegExp(`\\b${any}\\b`).test(l)) {
...
/** A node standing for a whole file rather than a symbol inside one. */
export const isFileNode = (n) => !!n && n.name === n.file.split(/[\\/]/).pop();
...
function neighbours(g, id, which, repo, cache) {
  ...
  for (const e of edges) {
    ...
    if (repo) {
      ...
      // A graph carries file nodes as well as symbol nodes, and a file's edge
      // to the symbols inside it means containment, not a call. There is no
      // call site to look at, so checking for one would reject every last
      // structural edge in the graph.
      if (!isFileNode(node) && !isFileNode(self)) {
...
export function findSymbols(g, task, max = 6) {
  // An unindexed graph knows a symbol's name and where it lives, and nothing
  // about what it is for — there is no doc comment to match against until the
  // index has been built. Splitting identifiers and trimming word endings is
  // therefore all that is available here, and it is worth doing: "orders"
  // reaches placeOrder, which raw substring matching never did.
  const terms = queryTerms(task);
  if (!terms.length) return [];
  const scored = [];
  for (const n of g.nodes.values()) {
    const score = lexicalScore(terms, { name: n.label || n.name, file: n.file });
