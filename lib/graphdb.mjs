 ...
 */
/**
 * Every symbol row for a repo, unfiltered and unranked.
 *
 * Exists for callers building their own ranking on top of the same data
 * `searchSymbols` reads — keel's index-time-tagging experiment, first — so
 * they don't reach past this module's export list to do it. Same columns
 * `searchSymbols` selects; kept as one query so the two never drift apart.
 */
export function symbolRows(repo) {
  const db = d();
  const R = key(repo);
  return db.prepare(
