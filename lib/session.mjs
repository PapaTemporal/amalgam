...
/**
 * GitHub Copilot's CLI, if it is here.
 *
 * Detected but never driven. amalgam runs sessions by speaking Claude Code's
 * streaming protocol on stdin and stdout; Copilot is a different program with
 * a different one, and pretending otherwise would fail at the first turn.
 *
 * It is worth knowing about anyway, because `amalgam wire --copilot` sets up
 * the MCP server for it: somebody with Copilot gets every tool here inside
 * their own editor. What they do not get is Start work, and being told "no
 * agent CLI on this machine" when they are looking at one is a poor way to
 * learn that.
 */
export function copilotCli() {
  const r = spawnSync(WIN ? "where" : "which", ["copilot"], { encoding: "utf8", windowsHide: true });
  if (r.status !== 0) return null;
  const found = (r.stdout ?? "").split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  return (WIN ? realExecutable(found) ?? found[0] : found[0]) ?? null;
}
