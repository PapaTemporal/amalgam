...
export function graphSummary(dir) {
  ...
  if (!isWorkspace(repo)) {
    ...
    return status
      ? { symbols: status.symbols, edges: status.edges, importedAt: status.imported_at, services: null }
      : null;
  }
...
export function searchWorkspace(dir, task, opts = {}) {
  ...
  const scored = hits.filter((h) => h.score != null).sort((a, b) => b.score - a.score);
  const rest = hits.filter((h) => h.score == null);
