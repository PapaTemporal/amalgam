...
/**
 * A worktree of a repository, rather than a repository.
 *
 * Both look like a checkout from the outside, and a work stream lands beside
 * the repository it came from — which, for a service inside a workspace, is
 * inside the workspace. Counted as a service it would contribute its parent's
 * symbols a second time, run its parent's checks again, and put a duplicate of
 * every file into the project's graph.
 *
 * The tell is on disk and needs no registry: a worktree's `.git` is a file
 * pointing into `.git/worktrees/`. A submodule uses a file too, pointing into
 * `.git/modules/`, and that IS a repository somebody may want as a service —
 * so the target is read rather than the file merely being counted.
 */
function isWorktree(dir) {
  const dotgit = path.join(dir, ".git");
  let st;
  try { st = fs.statSync(dotgit); } catch { return false; }
  if (st.isDirectory()) return false;
  try {
    const target = fs.readFileSync(dotgit, "utf8").replace(/\\/g, "/");
    return /^gitdir:/i.test(target.trim()) && target.includes("/worktrees/");
  }
  catch { return false; }
}
...
export function services(dir) {
  ...
  return entries
    ...
    .filter((e) => !isWorktree(path.join(dir, e.name)))
...
export function workspaceGraph(dir) {
  const parts = services(dir)
    .map((s) => ({ service: s, graph: graphFromDb(s.path) }))
...
export function graphFor(dir) {
  ...
  // The index is the only place a graph is read from. graphify's graph.json is
  // its output file, consumed once by importGraph and never again: keeping it
  // readable here made one dataset into two live representations, each with its
  // own reader, and left every caller choosing between them. hasGraph() still
  // reports whether that file exists, because "built but never indexed" is a
  // real state worth naming — it just is not a state anything answers from.
...
export function graphSummary(dir) {
  ...
  return {
    ...
    // "Has a graph file but no index" is no longer a state a service can be
    // in: `amalgam graph` consumes the extract as it indexes, so a service is
    // indexed or it has no graph, and there is one instruction either way.
    services: parts.map(({ name, indexed: ok, symbols, edges }) => ({
      name, indexed: ok, symbols, edges, needsGraph: !ok,
    })),
  };
}
