...
export function builtGraphs(projectRoot) {
  ...
  return parts.map((part) => {
    ...
    return {
      ...
      symbols,
      // Big enough that drawing it is worth warning about, not big enough to
      // refuse: graphify draws at the scale of communities, so even a hundred
      // thousand symbols renders in about a second once it has been built.
      slow: symbols > 20000,
