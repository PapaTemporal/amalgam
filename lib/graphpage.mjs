...
/**
 * Past this, the page is not drawn at all.
 *
 * graphify's own guidance, and easily confirmed: a browser asked to lay out a
 * hundred thousand nodes does not produce a slow diagram, it produces an
 * unusable one. The clustering still happens — the communities are real and
 * everything else uses them — but there is no picture, and saying "not drawn
 * yet" about a graph that will never be drawn is a promise that cannot be
 * kept.
 */
const TOO_BIG = 5000;
...
export function builtGraphs(projectRoot) {
  ...
  return parts.map((part) => {
    ...
    const symbols = indexStatus(part.path)?.symbols ?? 0;
    return {
      service: part.name,
      hasPage: fs.existsSync(page),
      hasReport: fs.existsSync(report),
      symbols,
      // Distinguished from "not drawn yet", because one of them is waiting on
      // a click and the other never will be.
      tooLarge: symbols > TOO_BIG,
