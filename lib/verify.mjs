 ...
 */
/**
 * Could this ever have been a path on this machine?
 *
 * An HTTP route reads exactly like a POSIX absolute path — `/api/state` — and
 * flagging a fact about an endpoint as "names a path that does not exist" is a
 * false positive that teaches people to ignore the warning. The distinction
 * that works without guessing: a real path's ROOT exists. `/etc/hosts` is
 * checkable on a machine that has `/etc`; `/api/state` is not, because nothing
 * called `/api` is there to hold it.
 */
function plausible(claim) {
  const full = resolve(claim.value);
  const root = full.split("/").filter(Boolean)[0];
  if (!root) return false;
  if (/^[A-Za-z]:$/.test(root)) return true;              // a drive letter is a path
  return fs.existsSync(full.startsWith("/") ? `/${root}` : root);
}

export function verifyFact(text) {
  const found = claims(text).filter(plausible);
