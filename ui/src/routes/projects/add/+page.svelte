<script>
  ...
  async function finish() {
    step = "running";
    const { jobId } = await post("/project/setup", { path: chosen, bmad, git: false });
  ...
  <div class="card err" style="margin-bottom:1rem">
    ...
    {#if isNew && info && !info.empty}
      <p class="tiny muted" style="margin:.4rem 0 0">
        <a href="/projects/add">Add it as an existing project</a> instead — that works with whatever is already there.
      </p>
    {/if}
  </div>
{/if}
